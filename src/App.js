...
class App extends Component {
	...
	render() {
		...
		const buttons = ['hour', 'day', 'week', 'month', 'year'].map((s, index) => {
			return (
				<button
					className={this.state.selectedScale === s ? 'active' : ''}
					onClick={() => {
						this.updateScale(s)
					}}
					key={index}
		...
		const currencyButtons = ['BTC', 'ETH', 'EOS', 'BCH', 'TRX'].map((c, index) => {
			return (
				<button
					className={this.state.selectedCurrency === c ? 'active' : ''}
					onClick={() => {
		...
		const conversionButtons = ['EUR', 'USD', 'JPY', 'GBP', 'RUB'].map((c, index) => {
			return (
				<button
					className={this.state.selectedConversion === c ? 'active' : ''}
					onClick={() => {
						this.updateConversion(c)
					}}
					key={index}
				>
					{c}
			...
			<div className="App">
				...
				<div className="title">
					<div className="container">
						...
						<p>Avatia is the perfect place to buy, sell and manage your cryptocurrency wallet.</p>
					</div>
				</div>
				<ListCurrency />
				<h2>Prix actuel</h2>
				...
				<div className="filters">{currencyButtons}</div>
				<div className="filters">{conversionButtons}</div>
